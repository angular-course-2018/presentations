...
import { debounceTime, switchMap, distinctUntilChanged, withLatestFrom, tap, catchError } from 'rxjs/operators';
...
export class AppEffects {
  ...
  @Effect() readonly firstnameEffect$ = this.actions$
    ...
    .pipe(
      ...
      switchMap(action => [
        new action.payload.ValueChangedAction(action.payload.value),
        // new FetchWeatherData(`${cityName}, ${countryCode}`)
      ])
    );
  ...
  @Effect() readonly fetchWeatherResults = this.actions$
    ...
    .pipe(
      ...
      switchMap(response => [new SetWeather(response)]),
      catchError((error, caught) => { console.log(error); return caught; }),
    );
}
